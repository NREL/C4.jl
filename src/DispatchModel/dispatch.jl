...
struct ThermalDispatch{G<:ThermalTechnology}
    ...
    function ThermalDispatch(
        ...
        min_up_time = @constraint(m, [t in 1:T],
            ...
            # sum(units_startup[tt] for tt in t:min(t - min_uptime(tech), T)) <= units_committed[t])
        ...
        min_down_time = @constraint(m, [t in 1:T],
            ...
            # sum(units_shutdown[tt] for tt in t:min(t - min_downtime(tech), T)) <= num_units(tech) - units_committed[t])
